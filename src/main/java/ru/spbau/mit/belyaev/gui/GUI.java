...
public class GUI extends JFrame {
    ...
    private GUI(String ipAddress) throws HeadlessException, IOException {
        ...
        uiLogger.log("Connected to server. \n");
    }

    public static void main(String[] args) {
        try {
            final String ipAddress = "localhost";
            final Server.Type[] serverTypes = {Server.Type.TCP_THREAD_POOL, Server.Type.UDP_THREAD_POOL};

            final GUI gui = new GUI(ipAddress);

            gui.performTest(serverTypes, TEST_LENGTH);
    ...
    private void performTest(Server.Type[] serverTypes, Supplier<IntSpan[]> testSupplier) throws IOException {
        ...
        for (Server.Type serverType : serverTypes) {
            final MainClient.TestResult testResult = mainClient.test(serverType, testSupplier.get());
